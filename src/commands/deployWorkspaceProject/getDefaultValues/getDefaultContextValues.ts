...
export async function getDefaultContextValues(
	context: ISubscriptionActionContext &
		Partial<DeployWorkspaceProjectContext>,
	...
	const rootFolder: WorkspaceFolder =
		context.rootFolder ?? (await getWorkspaceProjectRootFolder(context));
	const dockerfilePath: string =
		context.dockerfilePath ??
		nonNullValue(
			await selectWorkspaceFile(
				context,
				dockerFilePick,
				{ filters: {}, autoSelectIfOne: true },
				`**/${dockerfileGlobPattern}`,
			),
		);

	const settings: DeployWorkspaceProjectSettingsV1 =
		await dwpSettingUtilsV1.getDeployWorkspaceProjectSettings(rootFolder);
	dwpSettingUtilsV1.setDeployWorkspaceProjectSettingsTelemetry(
		context,
		settings,
	);
	...
	if (triggerSettingsOverride(settings, item)) {
		...
		await displaySettingsOverrideWarning(
			context,
			item as ContainerAppItem | ManagedEnvironmentItem,
		);
	...
	return {
		...
		environmentVariables:
			(await EnvironmentVariablesListStep.workspaceHasEnvFile())
				? undefined
				: [],
 ...
 */
export function triggerSettingsOverride(
	settings: DeployWorkspaceProjectSettingsV1,
	item: ContainerAppItem | ManagedEnvironmentItem | undefined,
): boolean {
	if (
		!item ||
		(!settings.containerAppName && !settings.containerAppResourceGroupName)
	) {
		return false;
...
async function displaySettingsOverrideWarning(
	context: ISubscriptionActionContext,
	...
	await context.ui.showWarningMessage(
		localize(
			...
			`Deployment will target {0} "{1}".\n\nAny workspace deployment settings will be skipped.  Would you like to proceed?`,
			nonNullValue(treeItemType),
			resourceName,
		),
		{ modal: true },
	...
	ext.outputChannel.appendLog(
		localize(
			"confirmedOverride",
			'User confirmed deployment will target {0} "{1}" instead of existing workspace settings.',
			nonNullValue(treeItemType),
			resourceName,
		),
	);
