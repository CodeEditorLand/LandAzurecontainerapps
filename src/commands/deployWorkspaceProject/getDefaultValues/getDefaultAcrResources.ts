...
export async function getDefaultAcrResources(
	...
	if (
		...
		triggerSettingsOverride(settings, item)
	) {
		return noMatchingResource;
	}

	const registries: Registry[] = await AcrListStep.getRegistries(context);
	const savedRegistry: Registry | undefined = registries.find(
		(r) => r.name === settings.containerRegistryName,
	);
	...
	if (savedRegistry) {
		ext.outputChannel.appendLog(
			localize(
				"foundResourceMatch",
				"Used saved workspace settings and found an existing container registry.",
			),
		);
		return {
			registry: savedRegistry,
			imageName: ImageNameStep.getTimestampedImageName(
				settings.containerAppName || nonNullProp(savedRegistry, "name"),
			),
		};
	} else {
		ext.outputChannel.appendLog(
			localize(
				"noResourceMatch",
				'Used saved workspace settings to search for Azure Container Registry "{0}" but found no match.',
				settings.containerRegistryName,
			),
		);
		return noMatchingResource;
