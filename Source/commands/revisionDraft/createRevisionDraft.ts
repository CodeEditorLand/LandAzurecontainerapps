...
export async function createRevisionDraft(
	...
	if (
		containerAppsItem.containerApp.revisionsMode !==
		KnownActiveRevisionsMode.Multiple
	) {
		throw new Error(
			localize(
				"revisionsModeError",
				"You must be in multiple revisions mode to run this command.",
			),
		);
	} else if (
		...
		throw new Error(
			localize(
				"revisionDraftExists",
				'A revision draft already exists for container app "{0}".',
				containerAppsItem.containerApp.name,
			),
		);
	}
...
async function promptForRevisionName(
	...
	return (
		await context.ui.showQuickPick(revisionPicks, {
...
async function getRevisionNamePicks(
	context: IContainerAppContext,
): Promise<IAzureQuickPickItem<Revision | undefined>[]> {
	...
	const revisionsIterator = client.containerAppsRevisions.listRevisions(
		rgName,
		caName,
	);
		...
		return [
			{
				...
				data: revisions.find(
					(revision: Revision) =>
						revision.name ===
						context.containerApp?.latestRevisionName,
				),
			},
		];
	}
	...
	return revisions
		.map((revision: Revision) => {
			...
			return {
				label:
					revisionName === context.containerApp?.latestRevisionName
						? `${revisionName}`
						: revisionName,
				description:
					day && time && timeAgo
		...
		.sort(
			(
				a: IAzureQuickPickItem<Revision>,
