...
import { type Progress, ProgressLocation, window } from "vscode";
...
export abstract class RevisionDraftUpdateBaseStep<
...
> extends AzureWizardExecuteStep<T> {
	...
	protected async updateRevisionDraftWithTemplate(context: T): Promise<void> {
		ext.revisionDraftFileSystem.updateRevisionDraftWithTemplate(
			this.baseItem,
			this.revisionDraftTemplate,
		);
	...
	private initRevisionDraftTemplate(): Template {
		let template: Template | undefined =
			ext.revisionDraftFileSystem.parseRevisionDraft(this.baseItem);
		if (!template) {
	...
	private async showRevisionDraftDeployPopup(context: T): Promise<void> {
		if (
			!(await settingUtils.getGlobalSetting(
				"showDraftCommandDeployPopup",
			))
		) {
			return;
		}
		...
		const dontShowAgain: string = localize(
			"dontShowAgain",
			"Don't show again",
		);
		...
		void window
			...
			.then(async (result: string | undefined) => {
				if (result === yes) {
					const item: ContainerAppItem | RevisionDraftItem =
						await window.withProgress(
							...
							async () => {
								const containerAppItem: ContainerAppItem =
									await pickContainerAppWithoutPrompt(
										context,
										this.baseItem.containerApp,
										{ showLoadingPrompt: false },
									);

								if (
									this.baseItem.containerApp.revisionsMode ===
									KnownActiveRevisionsMode.Single
								) {
									return containerAppItem;
								} else {
									return await pickRevisionDraft(
										context,
										containerAppItem,
										{ showLoadingPrompt: false },
									);
					...
					await callWithTelemetryAndErrorHandling(
						"revisionDraftUpdateBaseStep.deploy",
						async (context: IActionContext) =>
							await deployRevisionDraft(context, item),
					);
				} else if (result === dontShowAgain) {
					await settingUtils.updateGlobalSetting(
						"showDraftCommandDeployPopup",
						false,
					);
				} else {
