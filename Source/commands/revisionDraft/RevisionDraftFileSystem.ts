...
export class RevisionDraftFile implements FileStat {
	...
	commandUpdatesCount = 0; // Updates via revision draft commands
	directUpdatesCount = 0; // Direct updates via 'editContainerApp' & 'editDraft'
...
export class RevisionDraftFileSystem implements FileSystemProvider {
	...
	private buildUriFromItem(item: ContainerAppsItem): Uri {
		...
		const shortenedContainerAppId: string = `${parsedResourceId.subscriptionId?.slice(
			0,
			5,
		)}.../resourceGroups/${parsedResourceId.resourceGroup}/containerApps/${
