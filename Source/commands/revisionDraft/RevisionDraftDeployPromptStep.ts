...
> extends AzureWizardPromptStep<T> {
	...
	public async prompt(context: T): Promise<void> {
		...
		const dontAskAgain: string = localize(
			"dontAskAgain",
			"No, don't ask again",
		);
		...
		if (result === yes) {
			...
			if (result === dontAskAgain) {
				await settingUtils.updateGlobalSetting(
					showDraftCommandDeployPopupSetting,
					false,
				);
			}
		}
