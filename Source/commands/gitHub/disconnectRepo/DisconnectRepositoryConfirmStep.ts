...
export class DisconnectRepositoryConfirmStep extends AzureWizardPromptStep<IDisconnectRepoContext> {
	public async prompt(context: IDisconnectRepoContext): Promise<void> {
		...
		await context.ui.showWarningMessage(
			localize(
				"disconnectRepositoryWarning",
				'Disconnect from GitHub repository "{0}"? \n\nThis will not affect your container app\'s active deployment. You may reconnect a repository at any time.',
				`${owner}/${repo}`,
			),
			{ modal: true },
