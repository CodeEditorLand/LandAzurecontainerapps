...
export class GitHubRepositoryDisconnectStep extends AzureWizardExecuteStep<IDisconnectRepoContext> {
	...
	public async execute(
		...
		const { ownerOrOrganization: owner, repositoryName: repo } =
			gitHubUrlParse(
				nonNullValueAndProp(context.sourceControl, "repoUrl"),
			);
		...
		const requestOptions = {
			customHeaders: {
				"x-ms-github-auxiliary": nonNullProp(
					context,
					"gitHubAccessToken",
				),
			},
		...
		const disconnecting: string = localize(
			"disconnectingRepository",
			'Disconnecting "{0}"...',
			repo,
		);
		...
		const disconnected: string = localize(
			...
			'Disconnected repository "{0}" from container app "{1}".',
			`${owner}/${repo}`,
