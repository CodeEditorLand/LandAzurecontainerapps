...
export namespace dwpSettingUtilsV1 {
	...
	export async function getDeployWorkspaceProjectSettings(
		rootFolder: WorkspaceFolder,
	): Promise<DeployWorkspaceProjectSettingsV1> {
		const settingsPath: string =
			settingUtils.getDefaultRootWorkspaceSettingsPath(rootFolder);

		const containerAppName: string | undefined =
	 ...
	 */
	export async function setDeployWorkspaceProjectSettings(
		rootFolder: WorkspaceFolder,
	...
	export function displayDeployWorkspaceProjectSettingsOutput(
		settings: DeployWorkspaceProjectSettingsV1,
	): void {
		if (hasAllDeployWorkspaceProjectSettings(settings)) {
			...
			ext.outputChannel.appendLog(
				localize(
					"resourceSettingsIncomplete",
					'Found incomplete container app workspace settings at "{0}".',
			...
			ext.outputChannel.appendLog(
				localize(
					"noWorkspaceSettings",
					'Found no container app workspace settings at "{0}".',
					relativeSettingsFilePath,
				),
			);
		}
	...
	export function hasAllDeployWorkspaceProjectSettings(
		settings: DeployWorkspaceProjectSettingsV1,
	): boolean {
		return (
	...
	export function hasAtLeastOneDeployWorkspaceProjectSetting(
		settings: DeployWorkspaceProjectSettingsV1,
	): boolean {
		return (
			!!settings.containerAppName ||
	...
	export function hasNoDeployWorkspaceProjectSettings(
		settings: DeployWorkspaceProjectSettingsV1,
	): boolean {
		return (
			!settings.containerAppName &&
