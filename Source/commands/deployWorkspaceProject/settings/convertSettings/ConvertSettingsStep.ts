...
export class ConvertSettingsStep extends AzureWizardExecuteStep<ConvertSettingsContext> {
	...
	public async execute(context: ConvertSettingsContext): Promise<void> {
		const settingsContentsV1: DeployWorkspaceProjectSettingsV1 =
		...
		if (
			settingsContentsV2?.length ||
			(!settingsContentsV1.containerAppResourceGroupName &&
				!settingsContentsV1.containerAppName &&
				!settingsContentsV1.containerRegistryName)
		) {
			return;
		}
		...
		ext.outputChannel.appendLog(
			localize(
				"convertedSettings",
				"Detected deprecated deployment settings. Automatically converting settings to the latest workspace deployment schema.",
			),
		);
	}
