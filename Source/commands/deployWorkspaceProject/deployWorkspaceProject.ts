...
export async function deployWorkspaceProject(
	...
	ext.outputChannel.appendLog(
		formatSectionHeader(
			localize(
				...
				"Prepare workspace deployment configuration",
			),
		),
	);
	...
	if (item) {
		ext.outputChannel.appendLog(
			localize(
				...
				'Loading deployment configuration from user provided tree item "{0}".',
	...
	context.telemetry.properties.choseExistingWorkspaceConfiguration =
		deploymentConfiguration.configurationIdx !== undefined
			? "true"
			: "false";

	const deployWorkspaceProjectInternalContext: DeployWorkspaceProjectInternalContext =
		Object.assign(containerAppContext, {
	...
	void window
		...
		.then(async (result: string | undefined) => {
			await callWithTelemetryAndErrorHandling(
				"deployWorkspaceProject.displayNotification",
				async (
					telemetryContext: IActionContext &
						SetTelemetryProps<NotificationTelemetryProps>,
				) => {
