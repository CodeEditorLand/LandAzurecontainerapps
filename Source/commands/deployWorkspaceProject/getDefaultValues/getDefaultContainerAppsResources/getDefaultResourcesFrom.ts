...
import {
	getResourcesFromContainerAppHelper,
	getResourcesFromManagedEnvironmentHelper,
} from "./getResourceHelpers";
...
export async function getContainerAppResourcesFromSettings(
	...
	try {
		...
		const containerAppModel: ContainerAppModel =
			ContainerAppItem.CreateContainerAppModel(containerApp);

		const resources: DefaultContainerAppsResources =
			await getResourcesFromContainerAppHelper(
				context,
				containerAppModel,
			);
		ext.outputChannel.appendLog(
			localize(
				"foundResourceMatch",
				"Used saved workspace settings and found existing container app resources.",
			),
		);
		...
		ext.outputChannel.appendLog(
			localize(
				"noResourceMatch",
				'Used saved workspace settings to search for container app "{0}" in resource group "{1}" but found no match.',
				settings.containerAppName,
				settings.containerAppResourceGroupName,
			),
		);
		return noMatchingResources;
	}
...
export async function getContainerAppResourcesFromItem(
	context: ISubscriptionActionContext,
	item: ContainerAppItem | ManagedEnvironmentItem,
): Promise<DefaultContainerAppsResources> {
	if (ContainerAppItem.isContainerAppItem(item)) {
		return await getResourcesFromContainerAppHelper(
			context,
			item.containerApp,
		);
	} else if (ManagedEnvironmentItem.isManagedEnvironmentItem(item)) {
		return await getResourcesFromManagedEnvironmentHelper(
			context,
			item.managedEnvironment,
		);
	} else {
		const incompatibleMessage: string = localize(
			"incompatibleTreeItem",
			"An incompatible tree item was provided to Azure Container Apps for project deployment.",
		);
		ext.outputChannel.appendLog(
			localize(
				"incompatibleMessageLog",
				"Error: {0}",
				incompatibleMessage,
			),
		);
