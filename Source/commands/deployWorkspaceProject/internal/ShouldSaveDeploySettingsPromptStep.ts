...
export class ShouldSaveDeploySettingsPromptStep extends AzureWizardPromptStep<DeployWorkspaceProjectInternalContext> {
	public async prompt(
		...
		if (context.configurationIdx !== undefined) {
			const rootFolder: WorkspaceFolder = nonNullProp(
				context,
				"rootFolder",
			);
			...
			const hasNewSettings: boolean =
				...
				(!!context.resourceGroup &&
					setting?.resourceGroup !== context.resourceGroup.name) ||
				(!!context.containerApp &&
					setting?.containerApp !== context.containerApp.name) ||
				(!!context.registry &&
					setting?.containerRegistry !== context.registry.name);
...
function convertRelativeToAbsolutePath(
	rootPath: string,
