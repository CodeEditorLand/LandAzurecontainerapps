	...
	public async configureBeforePrompt(
		...
		if (
			...
			if (!context.newRegistryName) {
				...
				throw new Error(
					localize(
						"failedToGenerateName",
						"Failed to generate an available container registry name.",
					),
				);
			}
		}
	...
	public async prompt(
		...
		const resourceName: string = (
			await context.ui.showInputBox({
				prompt: localize(
					"sharedNamePrompt",
					"Enter a name for the container app environment",
				),
				value: sanitizeResourceName(context.rootFolder?.name ?? ""),
				validateInput: (name: string) =>
					this.validateInput(context, name),
		...
		ext.outputChannel.appendLog(
			localize(
				"usingSharedName",
				'User provided the name "{0}" for the container app environment.',
				resourceName,
			),
		);
