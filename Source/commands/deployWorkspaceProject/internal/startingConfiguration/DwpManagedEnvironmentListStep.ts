...
const recommendedPickDescription: string = localize(
	"recommended",
	"(Recommended)",
);
export class DwpManagedEnvironmentListStep extends AzureWizardPromptStep<DeployWorkspaceProjectInternalContext> {
	public async prompt(
		...
		await this.setContextWithManagedEnvironmentResources(
			context,
			managedEnvironment,
		);
	}
		...
		const managedEnvironments: ManagedEnvironment[] =
			await uiUtils.listAllIterator(
				context.resourceGroup
					? client.managedEnvironments.listByResourceGroup(
		...
		return [
			{
				label: localize(
					...
					"$(plus) Create new container apps environment",
				),
				data: undefined,
			},
	...
	private async setContextWithManagedEnvironmentResources(
		context: DeployWorkspaceProjectInternalContext,
	...
	private async setRecommendedPicks(
		context: DeployWorkspaceProjectInternalContext,
		picks: IAzureQuickPickItem<ManagedEnvironment | undefined>[],
	): Promise<void> {
		...
		for (const config of deploymentConfigurations) {
			try {
				if (config.resourceGroup && config.containerApp) {
					const containerApp = await client.containerApps.get(
...
const isRecommendedPick = (
	pick: IAzureQuickPickItem<ManagedEnvironment | undefined>,
): boolean => pick.description === recommendedPickDescription;
