...
} from "./containerAppsResourceHelpers";
...
export async function getStartingConfiguration(
	...
	return {
		...
		environmentVariables: context.envPath
			...
			: (await EnvironmentVariablesListStep.workspaceHasEnvFile(
				...
				? undefined
				: [] /** The equivalent of "skipForNow" */,
	};
}

async function tryAddMissingAzureResourcesToContext(
	context: DeployWorkspaceProjectInternalContext,
): Promise<void> {
	if (!context.containerApp && !context.managedEnvironment) {
		return;
	} else if (context.containerApp) {
		const resources = await getResourcesFromContainerAppHelper(
			context,
