...
export class DeploymentConfigurationListStep extends AzureWizardPromptStep<WorkspaceDeploymentConfigurationContext> {
	public async prompt(
		...
		const pick = await context.ui.showQuickPick(
			this.getPicks(deploymentConfigurations),
			{
				placeHolder: localize(
					"chooseDeployConfigurationSetting",
					"Select an app configuration to deploy",
				),
	...
	public async getSubWizard(
		...
		IWizardOptions<WorkspaceDeploymentConfigurationContext> | undefined
	> {
		if (!context.deploymentConfigurationSettings) {
			ext.outputChannel.appendLog(
				localize(
					"createNewAppConfiguration",
		...
		if (context.deploymentConfigurationSettings.label) {
			ext.outputChannel.appendLog(
				localize(
					"choseExistingConfiguration",
					'User chose to load existing workspace deployment configuration "{0}".',
			...
			ext.outputChannel.appendLog(
				localize(
					"choseExistingConfiguration",
					"User chose to load existing workspace deployment configuration.",
				),
			);
		}
	...
	private getPicks(
		...
		| (DeploymentConfigurationSettings & { configurationIdx?: number })
		| undefined
	>[] {
		const picks: IAzureQuickPickItem<
			DeploymentConfigurationSettings | undefined
		>[] = deploymentConfigurations.map((deploymentConfiguration, i) => {
			return {
				label:
					deploymentConfiguration.label ||
					localize("unnamedApp", "Unnamed app"),
				// Show the container app name as the description by default, unless the label has the same name
				description:
					deploymentConfiguration.label ===
					deploymentConfiguration.containerApp
						? undefined
						: deploymentConfiguration.containerApp,
		...
		picks.push({
			label: localize(
				...
				"$(plus) Create and deploy new app configuration",
			),
			data: undefined,
		});
