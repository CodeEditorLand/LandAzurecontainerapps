...
export async function getWorkspaceDeploymentConfiguration(
	context: IContainerAppContext & { rootFolder?: WorkspaceFolder },
): Promise<DeploymentConfiguration> {
	const wizardContext: WorkspaceDeploymentConfigurationContext =
		Object.assign(context, {
			...(await createActivityContext()),
		});

	const wizard: AzureWizard<WorkspaceDeploymentConfigurationContext> =
		new AzureWizard(wizardContext, {
			title: localize(
				"selectWorkspaceDeploymentConfigurationTitle",
				"Select a workspace deployment configuration",
			),
			promptSteps: [
				new RootFolderStep(),
	...
	if (wizardContext.deploymentConfigurationSettings) {
		wizardContext.activityTitle = wizardContext
			.deploymentConfigurationSettings.label
			? localize(
					"loadWorkspaceDeploymentActivityTitleOne",
					'Load workspace deployment configuration "{0}"',
					wizardContext.deploymentConfigurationSettings.label,
				)
			: localize(
					"loadWorkspaceDeploymentActivityTitleTwo",
					"Load workspace deployment configuration",
				);
	} else {
		wizardContext.activityTitle = localize(
			"prepareWorkspaceDeploymentActivityTitle",
			"Prepare new workspace deployment configuration",
		);
	}
