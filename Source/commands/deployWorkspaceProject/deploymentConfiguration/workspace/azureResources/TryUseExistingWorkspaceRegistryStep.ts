...
type TryUseExistingWorkspaceRegistryContext =
	WorkspaceDeploymentConfigurationContext & SetTelemetryProps<TelemetryProps>;

export class TryUseExistingWorkspaceRegistryStep<
	T extends TryUseExistingWorkspaceRegistryContext,
> extends AzureWizardExecuteStep<T> {
	public priority = 220; /** Todo: Figure out a good priority level */
	...
	public async execute(
		...
		if (context.deploymentConfigurationSettings) {
			...
			progress.report({
				message: localize(
					`verifyingContainerRegistry`,
					"Verifying container registry",
				),
			});
		} else {
			progress.report({
				message: localize(
					"searchingAvailableRegistries",
					"Searching available registries...",
				),
			});
		}
			...
			if (registryMap.has(setting.containerRegistry)) {
				...
				ext.outputChannel.appendLog(
					localize(
						...
						'Searched workspace settings and found an existing container registry "{0}" to leverage.',
