...
export abstract class FilePathsVerifyStep extends AzureWizardExecuteStep<WorkspaceDeploymentConfigurationContext> {
	...
	public async execute(
		...
		this.configPath = nonNullValueAndProp(
			context.deploymentConfigurationSettings,
			this.deploymentSettingskey,
		);
	...
	public createSuccessOutput(
		_: WorkspaceDeploymentConfigurationContext,
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"filePathVerifyStepSuccessItem",
					activitySuccessContext,
				]),
				label:
					this.fileType.charAt(0).toUpperCase() +
					this.fileType.slice(1) +
					" " +
					localize("path", "path"),
			...
			message: localize(
				"verifyPathSuccess",
				'Successfully verified {0} path "{1}".',
				this.fileType,
				this.configPath,
			),
		};
