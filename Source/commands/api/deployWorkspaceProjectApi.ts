...
import {
	...
	type DeployWorkspaceProjectResults,
} from "../deployWorkspaceProject/getDeployWorkspaceProjectResults";
...
export async function deployWorkspaceProjectApi(
	deployWorkspaceProjectOptions: api.DeployWorkspaceProjectOptionsContract,
): Promise<DeployWorkspaceProjectResults> {
	return (
		(await callWithTelemetryAndErrorHandling(
			"containerApps.api.deployWorkspaceProject",
			async (
				...
				const deployWorkspaceProjectInternalContext: DeployWorkspaceProjectInternalContext =
					Object.assign(context, {
						...subscriptionContext,
						subscription,
						resourceGroup,
						newManagedEnvironmentName:
							await tryGetNewManagedEnvironmentName(
								{ ...context, ...subscriptionContext },
								resourceGroup?.name,
								resourceGroup?.name,
							),
						...
						dockerfilePath: dockerfilePath
							? Uri.file(dockerfilePath).fsPath
							: undefined,
						shouldSaveDeploySettings: !!shouldSaveDeploySettings,
					});

				const deployWorkspaceProjectContext: DeployWorkspaceProjectContext =
					await deployWorkspaceProjectInternal(
						deployWorkspaceProjectInternalContext,
						{
...
async function tryGetNewManagedEnvironmentName(
	...
	if (
		!ManagedEnvironmentNameStep.validateInput(newEnvironmentName) &&
...
function getSubscriptionIdFromOptions(
	deployWorkspaceProjectOptions: api.DeployWorkspaceProjectOptionsContract,
): string | undefined {
	if (deployWorkspaceProjectOptions.subscriptionId) {
		...
		return parseAzureResourceGroupId(
			deployWorkspaceProjectOptions.resourceGroupId,
...
async function getResourceGroupFromId(
	context: ISubscriptionActionContext,
