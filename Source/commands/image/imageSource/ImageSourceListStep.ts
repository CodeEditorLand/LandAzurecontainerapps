	...
	public async prompt(context: ImageSourceContext): Promise<void> {
		...
		const imageSourceDetails: string[] = [
			localize(
				...
				"Use an image from Azure Container Registry or other third party container registry",
			),
			localize(
				"quickstartImageDetails",
				"Use our default starter image to quickly get a sample app up and running",
			),
			localize(
				"workspaceProjectDetails",
				"Build an image starting from a local workspace project with Dockerfile",
			),
		];
		...
		const picks: IAzureQuickPickItem<ImageSource | undefined>[] = [
			{
				...
				detail: imageSourceDetails[0],
				data: ImageSource.ContainerRegistry,
		...
		if (context.showQuickStartImage) {
			picks.unshift({
				label: imageSourceLabels[1],
				detail: imageSourceDetails[1],
				data: ImageSource.QuickstartImage,
				suppressPersistence: true,
			});
		}
		...
		if (env.uiKind === UIKind.Desktop && !isVirtualWorkspace) {
			picks.push({
				...
				detail: imageSourceDetails[2],
				data: ImageSource.RemoteAcrBuild,
				suppressPersistence: true,
			});
		}
