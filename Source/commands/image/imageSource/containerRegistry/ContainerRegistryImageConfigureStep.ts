...
import {
	getAcrCredentialsAndSecrets,
	getThirdPartyCredentialsAndSecrets,
} from "./getRegistryCredentialsAndSecrets";
...
export class ContainerRegistryImageConfigureStep extends AzureWizardExecuteStep<ContainerRegistryImageSourceContext> {
	...
	public async execute(
		...
		if (context.registryDomain === acrDomain) {
			// ACR
			const acrRegistryCredentialsAndSecrets =
			...
			if (context.registryName && context.username && context.secret) {
				const thirdPartyRegistryCredentialsAndSecrets =
					getThirdPartyCredentialsAndSecrets(context, {
						registries,
						secrets,
					});
				context.secrets =
					thirdPartyRegistryCredentialsAndSecrets.secrets;
				context.registries =
					thirdPartyRegistryCredentialsAndSecrets.registries;
			}
		}
		...
		context.image ||= `${getLoginServer(context)}/${
