...
export async function getAcrCredentialsAndSecrets(
	context: ContainerRegistryImageSourceContext,
	containerAppSettings?: RegistryCredentialsAndSecrets,
): Promise<RegistryCredentialsAndSecrets> {
	const registry = nonNullProp(context, "registry");
	const { username, password } = await listCredentialsFromRegistry(
		context,
		registry,
	);
	const passwordName = `${registry.name?.toLocaleLowerCase()}-${
		password?.name
	}`;

	// Remove duplicate registries
	const registries: RegistryCredentials[] =
		containerAppSettings?.registries?.filter(
			(r) => r.server !== registry.loginServer,
		) ?? [];
	registries?.push({
	...
	const secrets: Secret[] =
		containerAppSettings?.secrets?.filter((s) => s.name !== passwordName) ??
		[];
...
export function getThirdPartyCredentialsAndSecrets(
	context: ContainerRegistryImageSourceContext,
	containerAppSettings?: RegistryCredentialsAndSecrets,
): RegistryCredentialsAndSecrets {
	// If 'docker.io', convert to 'index.docker.io', else use registryName as loginServer
	const loginServer: string =
		context.registryDomain === dockerHubDomain
			? dockerHubRegistry
			: nonNullProp(context, "registryName").toLowerCase();
	const passwordSecretRef: string = `${loginServer.replace(
		/[^a-z0-9-]+/g,
		"",
	)}-${context.username}`;
	...
	// Remove duplicate secrets
	const secrets: Secret[] =
		containerAppSettings?.secrets?.filter(
			(s) => s.name !== passwordSecretRef,
		) ?? [];
	secrets?.push({
