...
export class AcrTagListStep extends RepositoryTagListStepBase {
	public async getPicks(
		...
		const client = createContainerRegistryClient(
			context,
		...
		const manifests: ArtifactManifestProperties[] =
			await uiUtils.listAllIterator(repoClient.listManifestProperties());

		const tags = manifests.reduce(
			(allTags: { tag: string; date: Date }[], m) => {
