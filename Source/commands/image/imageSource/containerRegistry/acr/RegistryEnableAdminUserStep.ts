...
export class RegistryEnableAdminUserStep extends AzureWizardPromptStep<ContainerRegistryImageSourceContext> {
	public async prompt(
		...
		const message = localize(
			"enableAdminUser",
			"An admin user is required to continue. If enabled, you can use the registry name as username and admin user access key as password to docker login to your container registry.",
		);
		await context.ui.showWarningMessage(
			message,
			{ modal: true },
			{ title: localize("enable", "Enable") },
		);
		...
		const updatedRegistry = await client.registries.beginUpdateAndWait(
