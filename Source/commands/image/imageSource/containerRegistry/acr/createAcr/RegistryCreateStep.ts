...
export class RegistryCreateStep extends AzureWizardExecuteStep<CreateAcrContext> {
	...
	public async execute(
		...
		progress.report({
			message: localize("creatingRegistry", "Creating registry..."),
		});
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"registryCreateStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"createRegistryLabel",
					'Create container registry "{0}"',
	...
	public createFailOutput(context: CreateAcrContext): ExecuteActivityOutput {
		return {
			...
			message: localize(
				"createRegistryFail",
				'Failed to create container registry "{0}".',
				context.newRegistryName,
			),
		};
	}
