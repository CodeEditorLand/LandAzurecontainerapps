...
export class RegistryNameStep extends AzureWizardPromptStep<CreateAcrContext> {
	public async prompt(context: CreateAcrContext): Promise<void> {
		context.newRegistryName = await context.ui.showInputBox({
			prompt: localize(
				"registryName",
				"Enter a name for the new registry",
			),
			validateInput: RegistryNameStep.validateInput,
			asyncValidationTask: (value: string): Promise<string | undefined> =>
				this.validateNameAvalability(context, value),
		});
	}
	...
	public static validateInput(name: string | undefined): string | undefined {
		...
		if (name.length < minLength || name.length > maxLength) {
			...
			return localize(
				"validateInputError",
				`The name must consist of alphanumeric characters.`,
			);
		}
	...
	private async validateNameAvalability(
		context: CreateAcrContext,
		name: string,
	) {
		const registryNameStatus = await RegistryNameStep.isNameAvailable(
			context,
			name,
		);
		...
		if (!registryNameStatus.nameAvailable) {
			return (
				...
				localize(
					"validateInputError",
					`The registry name ${name} is unavailable.`,
				)
			);
		}
	 ...
	 */
	public static async generateRelatedName(
		context: ISubscriptionActionContext,
		name: string,
	): Promise<string> {
		...
		do {
			...
			registryAvailable = !!(
				await RegistryNameStep.isNameAvailable(context, generatedName)
			).nameAvailable;
