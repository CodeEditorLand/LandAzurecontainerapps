...
export class AcrListStep extends AzureWizardPromptStep<ContainerRegistryImageSourceContext> {
	...
	public async getSubWizard(
		...
		IWizardOptions<ContainerRegistryImageSourceContext> | undefined
	> {
		if (!context.registry) {
			const promptSteps: AzureWizardPromptStep<ContainerRegistryImageSourceContext>[] =
				[new RegistryNameStep(), new SkuListStep()];
	...
	> {
		...
		if (context.containerApp) {
			...
			const srIndex: number = registries.findIndex(
				(r) =>
		...
		const suppressCreate: boolean =
			context.imageSource !== ImageSource.RemoteAcrBuild;
		if (!suppressCreate) {
			picks.push({
				label: localize(
		...
		return picks.concat(
			registries.map((r) => {
				return !!suggestedRegistry &&
					r.loginServer === suggestedRegistry
					? {
							label: nonNullProp(r, "name"),
							data: r,
							description: `${r.loginServer} ${currentlyDeployed}`,
							suppressPersistence: true,
						}
					: {
							label: nonNullProp(r, "name"),
							data: r,
							description: r.loginServer,
							suppressPersistence: srExists || !suppressCreate,
						};
			}),
		);
	}
...
async function tryConfigureResourceGroupForRegistry(
	...
	const resourceCreationContext =
		context as Partial<CreateContainerAppBaseContext> &
	...
	const resourceGroupName: string | undefined =
		resourceCreationContext.containerApp?.resourceGroup ||
