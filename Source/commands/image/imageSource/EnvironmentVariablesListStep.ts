...
import { envFileGlobPattern, ImageSource } from "../../../constants";
...
type EnvironmentVariablesContext = ImageSourceBaseContext &
...
export class EnvironmentVariablesListStep extends AzureWizardPromptStep<EnvironmentVariablesContext> {
	...
	public async prompt(context: EnvironmentVariablesContext): Promise<void> {
		...
		if (!context.envPath && existingData) {
			...
			context.environmentVariables =
				await this.parseEnvironmentVariablesFromEnvPath(
					context.envPath,
				);
		}
	...
	public async configureBeforePrompt(
		context: EnvironmentVariablesContext,
	): Promise<void> {
	...
	public shouldPrompt(context: EnvironmentVariablesContext): boolean {
		return (
			context.imageSource !== ImageSource.QuickstartImage &&
			context.environmentVariables === undefined
		);
	}

	private async promptForEnvPath(
		context: EnvironmentVariablesContext,
		showHasExistingData?: boolean,
		...
		return await selectWorkspaceFile(
			context,
			placeHolder,
			{
				filters: { "env file": ["env", "env.*"] },
				allowSkip: true,
				skipLabel,
			},
	...
	private async parseEnvironmentVariablesFromEnvPath(
		...
		return Object.keys(envData).map((name) => {
			return { name, value: envData[name] };
		});
	}

	public static async workspaceHasEnvFile(
	...
	private outputLogs(
		...
		context.telemetry.properties.setEnvironmentVariableOption =
			setEnvironmentVariableOption;
			...
			context.activityChildren?.push(
				new GenericTreeItem(undefined, {
					contextValue: createUniversallyUniqueContextValue([
						"environmentVariablesListStepSuccessItem",
						setEnvironmentVariableOption,
						activitySuccessContext,
					]),
					label: localize(
