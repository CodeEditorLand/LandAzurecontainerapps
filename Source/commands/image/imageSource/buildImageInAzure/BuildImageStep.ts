...
import { type MessageItem, ThemeColor, ThemeIcon, window } from "vscode";
...
import { type AcrBuildResults, openAcrBuildLogs } from "../../openAcrBuildLogs";
...
export class BuildImageStep extends ExecuteActivityOutputStepBase<BuildImageInAzureImageSourceContext> {
	...
	protected async executeCore(
		...
		if (outputImages) {
			...
			const logSasUrl = (
				await context.client.runs.getLogSasUrl(
			...
			const response: AzExtPipelineResponse =
				await sendRequestWithTimeout(
					context,
					{ method: "GET", url: nonNullValue(logSasUrl) },
					2500,
					undefined,
				);
			...
			void window
				.showErrorMessage(errorMessage, viewLogsButton)
				.then(async (result) => {
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createActivityChildContext([
					"buildImageStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"buildImageLabel",
					'Build image "{0}" in registry "{1}"',
					context.imageName,
					context.registryName,
				),
			...
			message: localize(
				"buildImageSuccess",
				'Finished building image "{0}" in registry "{1}".',
				context.imageName,
				context.registryName,
			),
		};
	...
	protected createFailOutput(
		...
		if (this.acrBuildError) {
			loadMoreChildrenImpl = () => {
				const buildImageLogsItem = new GenericTreeItem(undefined, {
					...
					label: localize(
						"buildImageLogs",
						"Click to view build image logs",
					),
		...
		return {
			...
			message: localize(
				"buildImageFail",
				'Failed to build image "{0}" in registry "{1}".',
				context.imageName,
				context.registryName,
			),
		};
