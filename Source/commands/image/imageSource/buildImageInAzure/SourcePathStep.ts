...
export class SourcePathStep extends AzureWizardPromptStep<BuildImageInAzureImageSourceContext> {
	public async prompt(
		...
		const srcPath: string | undefined = (
			await context.ui.showQuickPick(this.getPicks(context), {
				placeHolder: localize(
					"sourceDirectoryPick",
					"Choose your source code directory",
				),
	...
	private getPicks(
		...
		const directories: string[] = path
			.relative(rootPath, path.dirname(context.dockerfilePath))
			.split(path.sep);

		const picks: IAzureQuickPickItem<string | undefined>[] = [
			{ label: "." + path.sep, data: rootPath },
		];
	...
	private hasRootDockerfile(
		...
		return (
			path.relative(rootPath, context.dockerfilePath) ===
			path.basename(context.dockerfilePath)
		);
	}
}
