		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"logAnalyticsCreateStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"createWorkspace",
					'Create log analytics workspace "{0}"',
					context.newLogAnalyticsWorkspaceName ||
			...
			message: localize(
				"createLogAnalyticsWorkspaceSuccess",
				'Created log analytics workspace "{0}".',
				context.newLogAnalyticsWorkspaceName ||
					context.newManagedEnvironmentName,
			),
		};
	...
	public createFailOutput(
		...
		return {
			item: new GenericParentTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"logAnalyticsCreateStepFailItem",
					activityFailContext,
				]),
				label: localize(
					"createWorkspace",
			...
			message: localize(
				"createLogAnalyticsWorkspaceFail",
				'Failed to create log analytics workspace "{0}".',
				context.newLogAnalyticsWorkspaceName ||
					context.newManagedEnvironmentName,
			),
		};
