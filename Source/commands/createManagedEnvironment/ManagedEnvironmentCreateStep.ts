...
export class ManagedEnvironmentCreateStep extends AzureWizardExecuteStep<ManagedEnvironmentCreateContext> {
	...
	public async execute(
		...
		const resourceGroupName = nonNullValueAndProp(
		...
		context.managedEnvironment =
			await client.managedEnvironments.beginCreateOrUpdateAndWait(
				resourceGroupName,
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"managedEnvironmentCreateStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
			...
			message: localize(
				"createdManagedEnvironmentSuccess",
				'Created container apps environment "{0}".',
				context.newManagedEnvironmentName,
			),
		};
	}
	...
	public createFailOutput(
		...
		return {
			item: new GenericParentTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"managedEnvironmentCreateStepFailItem",
					activityFailContext,
				]),
				label: localize(
					"createManagedEnvironment",
			...
			message: localize(
				"createdManagedEnvironmentFail",
				'Failed to create container apps environment "{0}".',
				context.newManagedEnvironmentName,
			),
		};
	}
