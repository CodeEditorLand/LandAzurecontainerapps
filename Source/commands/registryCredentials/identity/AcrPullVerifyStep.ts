...
export class AcrPullVerifyStep extends AzureWizardExecuteStep<ManagedIdentityRegistryCredentialsContext> {
	...
	public async execute(
		...
		context.hasAcrPullRole = roleAssignments.some(
			(r) => !!r.roleDefinitionId?.endsWith(acrPullRoleId),
		);
	}
		...
		if (context.hasAcrPullRole) {
			return {
				item: new GenericTreeItem(undefined, {
					...
					label: localize(
						"verifyAcrPull",
						'Verify "{0}" access on container registry "{1}"',
						"acrPull",
				...
				message: localize(
					"verifyAcrPullSuccess",
					'Successfully verified "{0}" access on container registry "{1}".',
					"acrPull",
					context.registry?.name,
				),
			};
	...
	public createFailOutput(
		...
		return {
			...
			message: localize(
				"verifyAcrPullFail",
				'Failed to verify "{0}" access on container registry "{1}".',
				"acrPull",
				context.registry?.name,
			),
		};
