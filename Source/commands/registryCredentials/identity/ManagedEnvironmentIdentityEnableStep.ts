...
export class ManagedEnvironmentIdentityEnableStep extends AzureWizardExecuteStep<ManagedIdentityRegistryCredentialsContext> {
	...
	public shouldExecute(
		...
		return (
			!!context.managedEnvironment &&
			!context.managedEnvironment.identity?.principalId
		);
	}
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"managedEnvironmentIdentityEnableStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"enableIdentity",
					'Enable system-assigned identity for environment "{0}"',
					context.managedEnvironment?.name,
				),
				iconPath: activitySuccessIcon,
			}),
			message: localize(
				"enableIdentitySuccess",
				'Enabled system-assigned identity for environment "{0}"',
				context.managedEnvironment?.name,
			),
		};
	}
	...
	public createFailOutput(
		...
		return {
			item: new GenericParentTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"managedEnvironmentIdentityEnableStepFailItem",
					activitySuccessContext,
				]),
				label: localize(
					"enableIdentity",
			...
			message: localize(
				"enableIdentityFail",
				'Failed to enable system-assigned identity for environment "{0}"',
				context.managedEnvironment?.name,
			),
		};
	}
