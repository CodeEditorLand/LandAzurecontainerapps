...
export class AcrPullEnableStep extends AzureWizardExecuteStep<ManagedIdentityRegistryCredentialsContext> {
	...
	public async execute(
		...
		const roleCreateParams: RoleAssignmentCreateParameters = {
			...
			principalId: nonNullValueAndProp(
	...
	public createSuccessOutput(): ExecuteActivityOutput {
		return {
			item: new GenericTreeItem(undefined, {
				...
				label: localize(
					"enableAcrPull",
					'Grant "{0}" access to container environment identity',
					"acrPull",
				),
				iconPath: activitySuccessIcon,
			}),
			message: localize(
				"enableAcrPullSuccess",
				'Successfully granted "{0}" access to container environment identity.',
				"acrPull",
			),
		};
	}
	...
	public createFailOutput(): ExecuteActivityOutput {
		return {
			...
			message: localize(
				"enableAcrPullFail",
				'Failed to grant "{0}" access to container environment identity.',
				"acrPull",
			),
		};
	}
