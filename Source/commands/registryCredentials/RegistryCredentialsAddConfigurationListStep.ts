...
export class RegistryCredentialsAddConfigurationListStep extends AzureWizardPromptStep<RegistryCredentialsContext> {
	...
	public async configureBeforePrompt(
		...
		this.requiresRegistryConfiguration =
	...
	public async prompt(context: RegistryCredentialsContext): Promise<void> {
		context.newRegistryCredentialType = (
			await context.ui.showQuickPick(this.getPicks(context), {
				placeHolder: localize(
					"selectCredentialType",
					"Select a registry connection method",
				),
		...
		switch (context.newRegistryCredentialType) {
			case RegistryCredentialType.SystemAssigned:
				executeSteps.push(
					...
					new ManagedIdentityRegistryCredentialsAddConfigurationStep(
						registryDomain,
					),
				);
	...
	public async getPicks(
		...
		if (registryDomain === acrDomain) {
			picks.push({
				...
				detail: localize(
					"systemIdentityDetails",
					'Setup "{0}" access for container environment resources via a system-assigned identity',
					"acrPull",
				),
		...
		picks.push({
			...
			detail: localize(
				"dockerLoginDetails",
				"Setup docker login access to a registry via username and password.",
			),
