...
export class AcrEnableAdminUserStep extends AzureWizardExecuteStep<DockerLoginRegistryCredentialsContext> {
	...
	public async execute(
		...
		if (!context.registry?.adminUserEnabled) {
			throw new Error(
				localize(
					"failedToUpdate",
					'Failed to enable admin user for registry "{0}". Go to the portal to manually update.',
	...
	public createSuccessOutput(
		context: DockerLoginRegistryCredentialsContext,
	): ExecuteActivityOutput {
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"acrEnableAdminUserStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"enableAdminUser",
					'Enable admin user setting for container registry "{0}"',
					context.registry?.name,
				),
				iconPath: activitySuccessIcon,
			}),
			message: localize(
				"enableAdminUserSuccess",
				'Successfully enabled admin user setting for container registry "{0}".',
				context.registry?.name,
			),
		};
	}

	public createFailOutput(
		context: DockerLoginRegistryCredentialsContext,
	): ExecuteActivityOutput {
		return {
			item: new GenericParentTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"acrEnableAdminUserStepFailItem",
					activitySuccessContext,
				]),
				label: localize(
					"enableAdminUser",
			...
			message: localize(
				"enableAdminUserFail",
				'Failed to enable admin user setting for container registry "{0}".',
				context.registry?.name,
			),
		};
	}
