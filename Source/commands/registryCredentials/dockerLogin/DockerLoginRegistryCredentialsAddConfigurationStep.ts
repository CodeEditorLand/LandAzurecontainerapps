...
export class DockerLoginRegistryCredentialsAddConfigurationStep extends AzureWizardExecuteStep<DockerLoginRegistryCredentialsContext> {
	...
	constructor(
		private readonly supportedRegistryDomain:
			| SupportedRegistries
			| undefined,
	) {
		super();
	...
	private async getAcrCredentialAndSecret(
		context: DockerLoginRegistryCredentialsContext,
	...
	private getThirdPartyRegistryCredentialAndSecret(
		context: DockerLoginRegistryCredentialsContext,
	): RegistryCredentialAndSecret {
		// If 'docker.io', convert to 'index.docker.io', else use registryName as loginServer
		const loginServer: string =
			DockerLoginRegistryCredentialsAddConfigurationStep.getThirdPartyLoginServer(
				this.supportedRegistryDomain as
					| typeof dockerHubDomain
					| undefined,
	...
	public static getThirdPartyLoginServer(
		registryDomain: typeof dockerHubDomain | undefined,
		registryName: string,
	): string {
		return registryDomain === dockerHubDomain
			? dockerHubRegistry
