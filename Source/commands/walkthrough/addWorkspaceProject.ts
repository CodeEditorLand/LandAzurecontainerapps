...
import { type MessageItem, commands, workspace } from "vscode";
import { localize } from "../../utils/localize";

export async function addWorkspaceProjectWalkthrough(
	context: IActionContext,
): Promise<void> {
	if (workspace.workspaceFolders?.length) {
		const warning: string = localize(
			"workspaceWarning",
			"This tutorial requires that you start from an empty workspace.\n\nPress continue to reset your current workspace.",
		);
