...
export async function editContainer(
	...
	node?: ContainersItem | ContainerItem,
): Promise<void> {
	const item: ContainerItem | ContainersItem =
		node ?? (await pickContainer(context, { autoSelectDraft: true }));
	...
	const wizard: AzureWizard<ContainerEditContext> = new AzureWizard(
		wizardContext,
		{
			title: localize(
				"editContainer",
