...
export class ContainerImageEditDraftStep<
	T extends ContainerEditUpdateContext,
> extends RevisionDraftUpdateBaseStep<T> {
	...
	public async execute(
		...
		progress.report({
			message: localize("editingImage", "Editing image (draft)..."),
		});
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"containerImageEditDraftStepSuccessItem",
					activitySuccessContext,
				]),
				label: localize(
					"editImage",
					'Edit container image for app "{0}" (draft)',
					parentResource.name,
				),
			...
			message: localize(
				"editImageSuccess",
				'Successfully added image "{0}" to container app "{1}" (draft).',
				context.image,
				parentResource.name,
			),
		};
	...
	public createProgressOutput(): ExecuteActivityOutput {
		...
		return {
			item: new GenericTreeItem(undefined, {
				contextValue: createUniversallyUniqueContextValue([
					"containerImageEditDraftStepProgressItem",
					activityProgressContext,
				]),
				label: localize(
					"editImage",
	...
	public createFailOutput(context: T): ExecuteActivityOutput {
		...
		return {
			...
			message: localize(
				"editImageFail",
				'Failed to add image "{0}" to container app "{1}" (draft).',
				context.image,
				parentResource.name,
			),
		};
