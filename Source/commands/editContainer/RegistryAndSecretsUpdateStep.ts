...
export class RegistryAndSecretsUpdateStep<
...
> extends AzureWizardExecuteStep<T> {
	...
	public async execute(
		...
		if (
			this.areSecretsDeepEqual(
				containerAppEnvelope.configuration.secrets,
				context.secrets,
			) &&
			this.areRegistriesDeepEqual(
		...
		ext.outputChannel.appendLog(
			localize(
				"updatedSecrets",
				'Updated container app "{0}" with new registry secrets.',
				containerApp.name,
			),
		);
	}
	...
	private areSecretsDeepEqual(
		originalSecrets: Secret[] | undefined,
	...
	private areRegistriesDeepEqual(
		originalRegistries: RegistryCredentials[] | undefined,
		newRegistries: RegistryCredentials[] | undefined,
	): boolean {
...
function sortAlphabeticallyByKey<T extends Secret | RegistryCredentials>(
	a: T,
	b: T,
	key: keyof T,
): number {
