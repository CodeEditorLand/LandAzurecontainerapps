...
export class ScaleRangeUpdateStep<
	T extends ScaleRangeContext,
> extends RevisionDraftUpdateBaseStep<T> {
	public priority = 1110;
	...
	public async execute(context: T): Promise<void> {
		...
		ext.outputChannel.appendLog(
			localize(
				"updatedScaleRange",
				'Updated replica scaling range to {0}-{1} for "{2}".',
				context.newMinRange,
				context.newMaxRange,
				parentResourceName,
			),
