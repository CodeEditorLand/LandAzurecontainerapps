...
import {
	getParentResource,
	isTemplateItemEditable,
	TemplateItemNotEditableError,
} from "../../../utils/revisionDraftUtils";
...
export async function editScaleRange(
	context: IActionContext,
	node?: ScaleItem,
): Promise<void> {
	const item: ScaleItem =
		node ?? (await pickScale(context, { autoSelectDraft: true }));
	...
	const wizard: AzureWizard<ScaleRangeContext> = new AzureWizard(
		wizardContext,
		{
			title: localize(
				"editScaleRangePre",
				'Update replica scaling range for "{0}" (draft)',
				parentResource.name,
			),
			promptSteps: [
