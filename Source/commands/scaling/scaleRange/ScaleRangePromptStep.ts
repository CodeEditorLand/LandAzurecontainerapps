...
export class ScaleRangePromptStep extends AzureWizardPromptStep<ScaleRangeContext> {
	public async prompt(context: ScaleRangeContext): Promise<void> {
		const scaleRange: string = (
			await context.ui.showInputBox({
				prompt: localize(
					"editScalingRange",
					"Set the range of application replicas that get created in response to a scale rule. Set any range within the minimum of 0 and the maximum of 10 replicas",
				),
	...
	private validateInput(range: string | undefined): string | undefined {
		...
		if (!range || !formatRegex.test(range)) {
			return localize(
				"enterRange",
				'Please enter the range in the following format "0-10"',
			);
		}
		...
		if (min > 10 || max > 10) {
			return localize(
				"maxRangeExceeded",
				"The maximum number of replicas is 10.",
			);
		} else if (min > max) {
			return localize(
				"minHigherThanMax",
