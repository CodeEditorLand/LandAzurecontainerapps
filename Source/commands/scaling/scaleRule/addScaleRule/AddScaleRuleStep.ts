...
export class AddScaleRuleStep<
	T extends IAddScaleRuleContext,
> extends RevisionDraftUpdateBaseStep<T> {
	public priority = 1120;
	...
	private buildRule(context: T): ScaleRule {
		...
		switch (context.newRuleType) {
			case ScaleRuleTypes.HTTP:
				scaleRule.http = {
					metadata: {
						concurrentRequests: nonNullProp(
							context,
							"newHttpConcurrentRequests",
						),
					},
				};
				break;
					...
					auth: [
						{
							...
							triggerParameter: context.newQueueTriggerParameter,
						},
					],
				};
				break;
