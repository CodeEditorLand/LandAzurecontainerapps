...
export class ScaleRuleNameStep extends AzureWizardPromptStep<IAddScaleRuleContext> {
	...
	public async prompt(context: IAddScaleRuleContext): Promise<void> {
		context.newRuleName = (
			await context.ui.showInputBox({
				prompt: localize(
					"scaleRuleNamePrompt",
					"Enter a name for the new scale rule.",
				),
	...
	private validateInput(name: string | undefined): string | undefined {
		...
		if (!/^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/.test(name)) {
			return localize(
				"invalidChar",
				`A name must consist of lower case alphanumeric characters or '-', and must start and end with an alphanumeric character.`,
			);
		}
	...
	private async validateNameAvailable(
		...
		if (scaleRuleExists) {
			return localize(
				"scaleRuleExists",
				'The scale rule "{0}" already exists in container app "{1}". Please enter a unique name.',
				name,
				context.containerApp.name,
			);
		}
