...
export async function addScaleRule(
	context: IActionContext,
	node?: ScaleRuleGroupItem,
): Promise<void> {
	const item: ScaleRuleGroupItem =
		node ?? (await pickScaleRuleGroup(context, { autoSelectDraft: true }));
	...
	const wizard: AzureWizard<IAddScaleRuleContext> = new AzureWizard(
		wizardContext,
		{
			title: localize(
				"addScaleRuleTitle",
				'Add scale rule to "{0}" (draft)',
				parentResource.name,
			),
			promptSteps: [
	...
	wizardContext.activityTitle = localize(
		"addScaleRuleTitle",
		'Add {0} rule "{1}" to "{2}" (draft)',
		wizardContext.newRuleType,
		wizardContext.newRuleName,
		parentResource.name,
	);
