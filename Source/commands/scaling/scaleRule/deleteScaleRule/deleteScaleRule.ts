...
export async function deleteScaleRule(
	context: IActionContext,
	node?: ScaleRuleItem,
): Promise<void> {
	const item: ScaleRuleItem =
		node ?? (await pickScaleRule(context, { autoSelectDraft: true }));
	const { subscription, containerApp, revision } = item;

	const parentResource: ContainerAppModel | Revision = getParentResource(
		containerApp,
		revision,
	);
	...
	const confirmMessage = localize(
		"confirmMessage",
		"Are you sure you want to delete this scale rule?",
	);
	...
	const executeSteps: AzureWizardExecuteStep<ScaleRuleContext>[] = [
		new DeleteScaleRuleStep(item),
	];

	const wizard: AzureWizard<ScaleRuleContext> = new AzureWizard(
		wizardContext,
		{
			title: localize(
				"deleteScaleRuleTitle",
				'Delete scale rule from "{0}" (draft)',
				parentResource.name,
			),
			promptSteps,
			executeSteps,
