...
export class DeleteEnvironmentConfirmationStep extends AzureWizardPromptStep<IDeleteManagedEnvironmentContext> {
	...
	public async prompt(
		...
		if (deleteConfirmation === "ClickButton") {
			...
			); // no need to check result - cancel will throw error
		} else {
			const prompt: string = localize(
				"enterToDelete",
				'Enter "{0}" to delete this container apps environment. Deleting this will delete {1} container app(s) in this environment.',
				this.managedEnvironmentName,
				numOfResources,
			);
	...
	private isNameEqualToEnvironment(val: string | undefined): boolean {
		return (
			!!val &&
			val.toLowerCase() ===
				nonNullValue(this.managedEnvironmentName).toLowerCase()
		);
	}
}
