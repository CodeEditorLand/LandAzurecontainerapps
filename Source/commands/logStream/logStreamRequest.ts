...
export function getActiveLogStreams(
	...
	for (const [key, value] of logStreams) {
		if (
			...
			value.data.revision === context.revision?.name &&
			value.isConnected
		) {
	...
	if (activeStreams.size === 0) {
		throw new Error(
			localize("noActiveStreams", "There are no active log streams."),
		);
	}
...
export async function logStreamRequest(
	context: IStreamLogsContext,
): Promise<ILogStream> {
	...
	if (logStream && logStream.isConnected) {
		...
		return await new Promise(
			(onLogStreamCreated: (ls: ILogStream) => void): void => {
				void callWithTelemetryAndErrorHandling(
					"containerApps.streamingLogs",
					async (_context) => {
						...
						const logsResponse = await genericClient.sendRequest(
							createPipelineRequest({
								method: "GET",
								url: endpoint,
								abortSignal: abortController.signal,
								headers,
								streamResponseStatusCodes: new Set<number>([
									200,
								]),
							}),
						);
						...
						await new Promise<void>(
							(
								...
								logsResponse.readableStreamBody
...
export async function disconnectLogStreaming(
	context: IStreamLogsContext,
): Promise<void> {
	const allStreams = context.logStreamToStop
		? [context.logStreamToStop]
		: getActiveLogStreams(context);
	...
	for (const streams of allStreams.values()) {
		if (streams && streams.isConnected) {
			...
			await context.ui.showWarningMessage(
				localize(
					"alreadyDisconnected",
					"The log-streaming service is already disconnected.",
				),
			);
		}
	}
