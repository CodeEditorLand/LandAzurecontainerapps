...
export async function disableIngress(
	context: IActionContext,
	node?: ContainerAppsItem,
): Promise<void> {
	...
	if (!isIngressEnabled(wizardContext)) {
		throw new Error(
			localize(
				"ingressNotEnabled",
				'Ingress not currently enabled for container app "{0}".',
				containerApp.name,
			),
		);
	}
	...
	const title: string = localize(
		"disable",
		'Disable ingress for container app "{0}"',
		containerApp.name,
	);
	...
	const wizard: AzureWizard<IngressBaseContext> = new AzureWizard(
		wizardContext,
		{
			title,
			promptSteps,
