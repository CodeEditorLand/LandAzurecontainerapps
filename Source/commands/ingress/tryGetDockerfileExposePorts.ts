...
export async function tryGetDockerfileExposePorts(
	...
	for (const line of lines) {
		...
		for (const sp of singlePorts) {
			portRanges.push(new PortRange(Number.parseInt(sp)));
		}
		...
		for (const pr of portRange) {
			...
			portRanges.push(
				new PortRange(Number.parseInt(start), Number.parseInt(end)),
			);
		}
	}
