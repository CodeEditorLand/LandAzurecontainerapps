...
export class EnableIngressStep extends AzureWizardExecuteStep<IngressBaseContext> {
	...
	public async execute(
		...
		progress.report({
			message: localize("enablingIngress", "Enabling ingress..."),
		});
	...
	public createSuccessOutput(
		...
		return {
			...
			message: localize(
				"enableCompleted",
				'Enabled ingress on port {0} for container app "{1}".',
				context.targetPort,
				context.containerApp?.name,
			),
		};
	...
	public createFailOutput(
		...
		return {
			...
			message: localize(
				"enableIngressFailed",
				'Failed to enable ingress on port {0} for container app "{1}".',
				context.targetPort,
				context.containerApp?.name,
			),
		};
