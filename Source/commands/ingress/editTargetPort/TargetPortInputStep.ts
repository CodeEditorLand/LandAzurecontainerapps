...
export class TargetPortInputStep extends AzureWizardPromptStep<IngressContext> {
	public async prompt(context: IngressContext): Promise<void> {
		context.targetPort = Number(
			await context.ui.showInputBox({
				prompt: localize(
					"targetPort",
					"This is the port your container is listening on that will receive traffic. Set this value to the port number that your container uses.",
				),
	...
	public async configureBeforePrompt(context: IngressContext): Promise<void> {
		if (context.targetPort) {
			context.telemetry.properties.targetPort = String(
				context.targetPort,
			);
		}
	}
