...
export class ToggleIngressVisibilityStep extends IngressUpdateStepBase<IngressBaseContext> {
	...
	public async execute(
		...
		const warningPrompt = localize(
			"visibilityWarning",
			'This will change the ingress visibility from "{0}" to "{1}".',
			ingress.external
		...
		await this.updateIngressSettings(context, progress, {
			ingress,
			working,
			workCompleted,
		});
