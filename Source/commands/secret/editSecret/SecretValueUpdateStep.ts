...
export class SecretValueUpdateStep extends AzureWizardExecuteStep<ISecretContext> {
	...
	public async execute(
		...
		progress.report({
			message: localize("updatingSecret", "Updating secret..."),
		});

		let didUpdateSecret = false;
		...
		if (!didUpdateSecret) {
			throw new Error(
				localize(
					"noMatchingSecret",
					'No matching secret named "{0}" found for container app "{1}".',
					context.secretName,
					containerApp.name,
				),
			);
		...
		ext.outputChannel.appendLog(
			localize(
				"updatedSecretValue",
				'Updated secret value for "{0}" in container app "{1}".',
				context.secretName,
				containerApp.name,
			),
		);
