...
export class ChangeRevisionModeStep extends AzureWizardExecuteStep<IChooseRevisionModeContext> {
	...
	public async execute(
		...
		const changing: string = localize(
			"changingRevision",
		...
		await updateContainerApp(context, context.subscription, containerApp, {
			configuration: { activeRevisionsMode: context.newRevisionMode },
		});

		const changed: string = localize(
			"changedRevision",
			'Changed container app "{0}" to {1} revision mode.',
			containerApp.name,
			context.newRevisionMode?.toLowerCase(),
		);
