...
export async function executeRevisionOperation(
	context: IActionContext,
	node: ContainerAppItem | RevisionItem | undefined,
	...
	if (!node) {
		...
		if (
			node.containerApp.revisionsMode ===
	...
	await ext.state.runWithTemporaryDescription(
		item.id,
		revisionOperationDescriptions[operation],
		async () => {
...
const revisionOperationDescriptions = {
	...
	Record<keyof ContainerAppsAPIClient["containerAppsRevisions"], string>
>;
