...
export namespace validateUtils {
	...
	export function isValidLength(
		...
		if (
			...
			value.length < lowerLimitIncl ||
			value.length > upperLimitIncl
		) {
			return false;
		} else {
			return true;
	 ...
	 */
	export const getInvalidLengthMessage = (
		lowerLimitIncl?: number,
		upperLimitIncl?: number,
	): string => {
		if (!lowerLimitIncl && !upperLimitIncl) {
			...
			return localize(
				"invalidInputLength",
				"A valid input value is required to proceed.",
			);
		} else if (lowerLimitIncl && !upperLimitIncl) {
			return localize(
				"inputLengthTooShort",
				"The input value must be {0} characters or greater.",
				lowerLimitIncl,
			);
		} else if (!lowerLimitIncl && upperLimitIncl) {
			return localize(
				"inputLengthTooLong",
				"The input value must be {0} characters or less.",
				upperLimitIncl,
			);
		} else {
			return localize(
				"invalidBetweenInputLength",
				"The input value must be between {0} and {1} characters long.",
				lowerLimitIncl,
				upperLimitIncl,
			);
		}
	 ...
	 */
	export function isLowerCaseAlphanumericWithSymbols(
		value: string,
		symbols = "-",
		...
		const symbolPattern: string = symbols.replace(
		...
		const symbolsRepeatPattern: RegExp = /[^a-z0-9]{2}/g;
		return (
			pattern.test(value) &&
	 ...
	 * @param canSymbolsRepeat A boolean specifying whether or not repeating or consecutive symbols are allowed. Defaults to true.
	 */
	export function getInvalidLowerCaseAlphanumericWithSymbolsMessage(
		symbols = "-",
		canSymbolsRepeat = true,
	): string {
