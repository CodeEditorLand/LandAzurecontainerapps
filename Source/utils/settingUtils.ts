...
export namespace settingUtils {
	 ...
	 */
	export function getWorkspaceSettingFromAnyFolder(
		key: string,
		prefix: string = ext.prefix,
	): string | undefined {
		if (
	...
	export function getDefaultRootWorkspaceSettingsPath(
		rootWorkspaceFolder: WorkspaceFolder,
	): string {
		return path.join(
			rootWorkspaceFolder.uri.fsPath,
			vscodeFolder,
			settingsFile,
		);
	}

	function getLowestConfigurationLevel(
