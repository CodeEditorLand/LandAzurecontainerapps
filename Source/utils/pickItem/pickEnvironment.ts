...
import {
	AzExtResourceType,
	type ResourceGroupsTreeDataProvider,
...
export function getPickEnvironmentSteps(
	skipIfOne: boolean = false,
	subscriptionId?: string,
	environmentName?: string | RegExp,
): AzureWizardPromptStep<AzureResourceQuickPickWizardContext>[] {
	const tdp: ResourceGroupsTreeDataProvider =
	...
	if (environmentName) {
		environmentFilter =
			environmentName instanceof RegExp
				? environmentName
	...
	return [
		new QuickPickAzureSubscriptionStep(tdp, {
			selectBySubscriptionId: subscriptionId,
		}),
		...
		new ContextValueQuickPickStep(
			...
			},
			{
				placeHolder: localize(
					"selectContainerAppsEnvironment",
					"Select a container apps environment",
				),
				noPicksMessage: localize(
					"noContainerAppsEnvironment",
					"Current subscription has no container apps environments",
				),
			},
		),
	];
...
): Promise<ManagedEnvironmentItem> {
