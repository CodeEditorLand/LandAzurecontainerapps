 ...
 */
export class ParentResourceItemPickSteps<
	T extends AzureResourceQuickPickWizardContext,
> extends AzureWizardPromptStep<T> {
	...
	public async getSubWizard(
		...
		const containerAppItem: unknown =
			(lastNode as { branchItem?: ResourceModelBase })?.branchItem ??
			lastNode;

		if (!ContainerAppItem.isContainerAppItem(containerAppItem)) {
			throw new Error(
				localize(
					"expectedContainerAppItem",
					'Internal error: Expected last picked item to be a "ContainerAppItem".',
				),
			);
		}
