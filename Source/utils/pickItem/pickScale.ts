...
import {
	getPickRevisionDraftStep,
	getPickRevisionsStep,
	getPickRevisionStep,
} from "./pickRevision";
	...
	return new ContextValueQuickPickStep(
		...
		},
		{
			...
			noPicksMessage: localize(
				"noScaleRules",
				"Selected item has no scale rules",
			),
		},
	);
}

export async function pickScale(
	...
	return await runQuickPickWizard(
		context,
		{
			promptSteps: getPickScaleSteps(containerAppItem, {
				autoSelectDraft: options?.autoSelectDraft,
			}),
			title: options?.title,
		},
		containerAppItem,
...
export async function pickScaleRuleGroup(
	context: IActionContext,
	options?: RevisionDraftPickItemOptions,
): Promise<ScaleRuleGroupItem> {
	const containerAppItem: ContainerAppItem = await pickContainerApp(context);

	const promptSteps: AzureWizardPromptStep<QuickPickWizardContext>[] = [
		...getPickScaleSteps(containerAppItem, {
			autoSelectDraft: options?.autoSelectDraft,
		}),
...
export async function pickScaleRule(
	context: IActionContext,
	options?: RevisionDraftPickItemOptions,
): Promise<ScaleRuleItem> {
	const containerAppItem: ContainerAppItem = await pickContainerApp(context);
