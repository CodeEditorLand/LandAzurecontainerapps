...
export function getPickContainerSteps(
	options?: RevisionDraftPickItemOptions,
 ...
 */
export class ContainerItemPickSteps<
	...
	public async getSubWizard(
		...
		if (ContainerAppItem.isContainerAppItem(lastItem)) {
			...
			throw new Error(
				localize(
					"expectedItem",
					'Internal error: Expected last picked item to be a "ContainerAppItem", "RevisionItem", or "RevisionDraftItem".',
				),
			);
		}
