...
import {
	type PickItemOptions,
	type RevisionPickItemOptions,
} from "./PickItemOptions";
...
export function getPickRevisionDraftStep(): AzureWizardPromptStep<QuickPickWizardContext> {
	return new ContextValueQuickPickStep(
		...
		{
			contextValueFilter: {
				include: RevisionDraftItem.contextValueRegExp,
			},
			skipIfOne: true,
		},
	);
...
export function getPickRevisionStep(
	...
	return new ContextValueQuickPickStep(
		...
		},
		{
			...
			noPicksMessage: localize(
				"noRevisions",
				"Selected container app has no revisions",
			),
		},
	);
}
...
export async function pickRevision(
	...
	startingNode?: ContainerAppItem | RevisionsItem,
	options?: RevisionPickItemOptions,
): Promise<RevisionItem> {
	startingNode ??= await pickContainerApp(context);

	if (
		startingNode.containerApp.revisionsMode ===
		KnownActiveRevisionsMode.Single
	) {
		throw new NoResourceFoundError(
			Object.assign(context, {
				noItemFoundErrorMessage: localize(
					"singleRevisionModeError",
					"Revision items do not exist in single revision mode.",
				),
			}),
		);
	}
...
export async function pickRevisionDraft(
	context: IActionContext,
	containerAppItem?: ContainerAppItem,
	options?: PickItemOptions,
): Promise<RevisionDraftItem> {
	...
	if (
		...
		throw new NoResourceFoundError(
			Object.assign(context, {
				noItemFoundErrorMessage: localize(
					"singleRevisionModeError",
					"Revision draft items do not exist in single revision mode.",
				),
			}),
		);
	}
	...
	if (
		...
		throw new NoResourceFoundError(
			Object.assign(context, {
				noItemFoundErrorMessage: localize(
					"noRevisionDraft",
					"Selected container app has no active revision draft.",
				),
			}),
		);
	}
