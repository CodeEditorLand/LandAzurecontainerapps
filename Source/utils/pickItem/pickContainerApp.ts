...
export function getPickContainerAppStep(
	containerAppName?: string | RegExp,
): AzureWizardPromptStep<AzureResourceQuickPickWizardContext> {
	let containerAppFilter: RegExp | undefined;

	if (containerAppName) {
		containerAppFilter =
			containerAppName instanceof RegExp
				? containerAppName
				: new RegExp(`^${containerAppName}$`);
	} else {
	...
	return new ContextValueQuickPickStep(
		...
		},
		{
			...
			noPicksMessage: localize(
				"noContainerApps",
				"Selected container apps environment has no apps",
			),
		},
	);
}
...
export function getPickContainerAppSteps(): AzureWizardPromptStep<AzureResourceQuickPickWizardContext>[] {
	return [...getPickEnvironmentSteps(), getPickContainerAppStep()];
}

export async function pickContainerApp(
...
export async function pickContainerAppWithoutPrompt(
	...
	const environmentName: string = parseAzureResourceId(
		nonNullProp(containerApp, "environmentId"),
	).resourceName;
