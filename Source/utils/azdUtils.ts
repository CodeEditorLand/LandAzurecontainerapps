...
import { type Extension, type WorkspaceFolder, extensions } from "vscode";
...
export function isAzdExtensionInstalled(): boolean {
	const azdExtension: Extension<unknown> | undefined =
		extensions.getExtension(azdExtensionId);
	// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
	return !!azdExtension?.packageJSON?.version;
}

export async function isAzdWorkspaceProject(
	rootFolder: WorkspaceFolder,
): Promise<boolean> {
	const azureYamlPath: string = path.join(
...
export async function addAzdTelemetryToContext(
