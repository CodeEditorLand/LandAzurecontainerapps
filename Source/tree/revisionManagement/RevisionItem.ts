...
export class RevisionItem implements RevisionsItemModel {
	...
	private get contextValue(): string {
		...
		values.push(
			this.revision.active
				...
				: revisionStateInactiveContextValue,
		);
	...
	getTreeItem(): TreeItem {
		return {
			id: this.id,
			label:
				this.revisionsMode === KnownActiveRevisionsMode.Single
					? "Latest"
					: this.revision.name,
	...
	static isRevisionItem(item: unknown): item is RevisionItem {
		return (
			...
			RevisionItem.contextValueRegExp.test(
				(item as RevisionItem).contextValue,
			)
		);
	}
}
