...
export class RevisionDraftItem
	implements RevisionsItemModel, RevisionsDraftModel
{
	...
	static isRevisionDraftItem(item: unknown): item is RevisionDraftItem {
		return (
			...
			(item as RevisionDraftItem).id.split("/").at(-1) ===
				RevisionDraftItem.idSuffix
		);
	}
	...
	async getTreeItem(): Promise<TreeItem> {
		return {
			...
			description:
				this.containerApp.latestRevisionName === this.revisionName
					? localize(
							"basedOnLatestRevision",
							'Based on "{0}" (Latest)',
							this.baseRevisionName,
						)
					: localize(
							"basedOnRevision",
							'Based on "{0}"',
	...
	async hasUnsavedChanges(): Promise<boolean> {
		const revisions: Revision[] | undefined =
			await callWithTelemetryAndErrorHandling(
				"revisionDraftItem.hasUnsavedChanges.getRevisions",
		...
		const baseRevisionName: string | undefined =
			ext.revisionDraftFileSystem.getRevisionDraftFile(
				this,
			)?.baseRevisionName;
