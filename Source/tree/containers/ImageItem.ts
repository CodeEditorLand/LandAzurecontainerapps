...
export class ImageItem extends RevisionDraftDescendantBase {
	...
	async getChildren(): Promise<TreeElementBase[]> {
		const {
			imageNameItem: isImageNameUnsaved,
			imageRegistryItem: isImageRegistryUnsaved,
		} = this.doChildrenHaveUnsavedChanges();

		return [
			createGenericElement({
				id: `${this.id}/imageName`,
				label: isImageNameUnsaved
					? localize("imageNameDraft", "Name*:")
					: localize("imageName", "Name:"),
			...
			createGenericElement({
				...
				label: isImageRegistryUnsaved
					? localize("imageRegistryDraft", "Registry*:")
					: localize("imageRegistry", "Registry:"),
	...
	private doChildrenHaveUnsavedChanges(): {
		...
		return {
			imageNameItem:
				this.getImageName(currentContainer.image) !==
				this.getImageName(this.container.image),
			imageRegistryItem:
				this.getLoginServer(currentContainer.image) !==
				this.getLoginServer(this.container.image),
		};
	}
