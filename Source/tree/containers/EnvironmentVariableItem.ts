...
export class EnvironmentVariableItem implements RevisionsItemModel {
	...
	getTreeItem(): TreeItem {
		return {
			label: this._hideValue
				? localize(
						"viewHidden",
						"{0}=Hidden value. Click to view.",
						this.envVariable.name,
					)
				: `${this.envVariable.name}=${this.envOutput}`,
			description:
				this.envVariable.secretRef && !this._hideValue
					? localize("secretRef", "Secret reference")
					: undefined,
