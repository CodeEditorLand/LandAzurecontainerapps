	...
	implements ContainerAppsItem, RevisionsDraftModel
{
	...
	private get contextValue(): string {
		...
		values.push(
			this.containerApp.revisionsMode === KnownActiveRevisionsMode.Single
				? revisionModeSingleContextValue
				: revisionModeMultipleContextValue,
		);
		values.push(
	...
	private get description(): string | undefined {
		...
		if (
			...
			this.containerApp.provisioningState !== "Succeeded"
		) {
	...
	async getChildren(): Promise<TreeElementBase[]> {
		const result = await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				if (
					this.containerApp.provisioningState === "Deleting" ||
					this.containerApp.provisioningState === "InProgress"
				) {
					throw new Error(
						localize(
							"provisioningError",
							'The container app "{0}" cannot be expanded due to its provisioning state of "{1}". Its children cannot be accessed until the app has finished provisioning.',
							this.containerApp.name,
							this.containerApp.provisioningState,
						),
					);
				...
				const client: ContainerAppsAPIClient =
					await createContainerAppsAPIClient([
						context,
				...
				if (
					...
					children.push(
						new RevisionsItem(this.subscription, this.containerApp),
					);
				}
	...
	static isContainerAppItem(item: unknown): item is ContainerAppItem {
		return (
			...
			ContainerAppItem.contextValueRegExp.test(
				(item as ContainerAppItem).contextValue,
			)
		);
	}
	...
	static async List(
		...
		return (
			...
			.filter(
				(ca) =>
					ca.managedEnvironmentId &&
					ca.managedEnvironmentId === managedEnvironmentId,
			)
	...
	static async Get(
		...
		const client: ContainerAppsAPIClient = await createContainerAppsClient(
			context,
			subscription,
		);

		return ContainerAppItem.CreateContainerAppModel(
			await client.containerApps.get(resourceGroupName, containerAppName),
		);
	}

	static CreateContainerAppModel(
		containerApp: ContainerApp,
	): ContainerAppModel {
		const revisionsMode =
			(containerApp.configuration
				?.activeRevisionsMode as KnownActiveRevisionsMode) ??
			KnownActiveRevisionsMode.Single;

		return {
			id: nonNullProp(containerApp, "id"),
			name: nonNullProp(containerApp, "name"),
			managedEnvironmentId: nonNullProp(
				containerApp,
				"managedEnvironmentId",
			),
			resourceGroup: getResourceGroupFromId(
				nonNullProp(containerApp, "id"),
			),
			revisionsMode,
			...containerApp,
		};
	...
	async delete(
		context: IActionContext & { suppressPrompt?: boolean },
	): Promise<void> {
		const confirmMessage: string = localize(
			"confirmDeleteContainerApp",
			'Are you sure you want to delete container app "{0}"?',
			this.name,
		);

		const deleteContainerApp: string = localize(
			"deleteContainerApp",
			'Delete container app "{0}"',
			this.name,
		);
		...
		const wizard: AzureWizard<IDeleteContainerAppWizardContext> =
...
export async function getContainerEnvelopeWithSecrets(
	context: IActionContext,
	subscription: AzureSubscription,
	containerApp: ContainerAppModel,
): Promise<Required<ContainerApp>> {
	...
	const containerAppEnvelope = <ContainerApp>(
		JSON.parse(JSON.stringify(containerApp))
	);
	...
	for (const key of Object.keys(containerAppEnvelope)) {
		...
		containerAppEnvelope[key] =
			containerAppEnvelope[<keyof ContainerApp>key];
	}

	const concreteContainerAppEnvelope = <Required<ContainerApp>>(
	...
	concreteContainerAppEnvelope.configuration.secrets = (
...
export function isIngressEnabled(
	containerApp: ContainerApp,
): containerApp is IngressEnabledContainerApp {
	return !!containerApp.configuration?.ingress?.fqdn;
}

type IngressEnabledContainerApp = ContainerApp & {
